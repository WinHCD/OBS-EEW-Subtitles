...
在 `styles.css` 文件中，也存在对应的默认颜色配置，作为 CSS 变量的 fallback 值：
...
这些颜色值是作为 CSS 变量的 fallback 值存在的，当 JavaScript 未加载或 CSS 变量未设置时会使用这些默认值。实际使用的颜色是从 `config.js` 中读取并通过 CSS 变量设置的，因此修改 `config.js` 中的颜色配置会自动更新页面显示。
